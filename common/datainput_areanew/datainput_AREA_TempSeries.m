function [IInormb,HS_SoX,HSurf,HLine] = datainput_AREA_TempSeries(user_sample_AREA,Temp)
...
if nargin<1;user_sample_AREA = 'user_datainputAREA';end	

%% Do NOT touch these, they set up empties used for defaults
[SPECpath,IMAGEpath,COMMONpath,HOMEpath] = pathdisplay;
NORMFLAG=0;	% use default - which is /sec*mon then scaled back to average mon
...
%fixfile;	%during run this is usually neccesary to reindex updated spec files
...
COLORMAPflag = 1;
COLORMAP = jet;
FORCE.FLAGdet = 4;  % 3 raw, 4 normaslized  then use norm that does all except filters
...
SoXFLAG = 0;  %% =1 will sums over delta in detector
...
% else
%     user_input_guts_AREA_Single;
% end

user_input_guts_AREA;
%user_input_guts_AREA_Single;

%user_input_guts_AREA_SingleScan_MultiFiles;
