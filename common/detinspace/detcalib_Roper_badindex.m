...
P2D1H = abs([tand(Halpha) ./ ( LENGTHXY(:,1).*cosdm(:,1) )]) 
P2D1V = abs([tand(Valpha) ./ ( LENGTHXY(:,2).*cosdm(:,2) )]) 

plot( [LENGTHXY(:,1)],abs([tand(Halpha) ./ cosdm(:,1)]),'o',[LENGTHXY(:,2)],abs([tand(Valpha) ./ cosdm(:,2) ]),'+');

diff([tand(Halpha) ./ cosdm(:,1)])./ diff([LENGTHXY(:,1)]);
diff([tand(Valpha) ./ cosdm(:,2)])./ diff([LENGTHXY(:,2)]);
...
P2D1H = sum( P2D1H .* LENGTHXY(:,1)./sum(LENGTHXY(:,1)) );
P2D1V = sum( P2D1V .* LENGTHXY(:,2)./sum(LENGTHXY(:,2)) );


% This is the operator to change Xpixels,Ypixel pairs to angles
det_s.rot.oper = [P2D1H 0; 0 P2D1V] * (DetRotOper');
...
% of angle, 
	...
	elseif	([( round(th_testMR1) == round(th_testMR2) ) | ...
		...
		disp(['If angle to pixel only requires a  rotation, all angles will be equal (in sign).']);
