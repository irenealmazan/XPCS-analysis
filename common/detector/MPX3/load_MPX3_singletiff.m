function [ImageStack,imtime] = load_MPX3(fullimname,MASK,badpixfile,SINGLE)
...
ImageStack = zeros([Yrow Xcol numel(SINGLE)]);

%%%%%%%  read in the images
ImInfo = imfinfo(fullimname(1,:));    %%% note recently the pilatus tiffs are not standard

for jj=1:numel(SINGLE);
	ImageStackii = imread(fullimname(SINGLE(jj),:),'tif');
