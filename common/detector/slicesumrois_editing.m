...
% [sum_over_rowsY,sum_over_columnsX] = slicesumrois(images,Rois,indxFLAG)
...
% % CT 2017-10 - plans editing this to handle 1 pixel wide ROIS
% % 	also to handle rotating the ROIs 
%
...
%		sumorowY.norm  is number of rows summed (to normalize the images sumorowY.image{1}./sumorowY.norm)
%
%
% Input
%	images	(array)
...
%
%	Rois    (if not a structure) then is a matrix
%			each row is an roi (lowX highX loY highY)
%			use showrois(ROIS) to show where the ROIS are on an image
%		(if a structure)
%		Rois.Rois = matrix, each row an roi (lowX highX loY highY)
%		Rois.Angle = vector, each element the rotation of the ROI about its center point 
%			alibi rotation rotate VECTOR (CCW about 'z')
%
%	indxFLAG (scalar)
%		= 1 (use imageJ type indexing, starting at [0:15] (16 pixels)
%		otherwise, use matlab type indexing, starting at [1:16] (16 pixels)
%
% CT note 2017-04 I need a more consistent way to handle the NaN pixels than how I am doing them
%   the fixNaNimage simply replaces them with zeros
% I should eliminate them from the sum and also fix the 'norm' to reflect it.
